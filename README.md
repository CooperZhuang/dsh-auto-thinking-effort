...
| **GUI 真机一轮** | 临时 `web-verify` profile（`dsh-base` + `dsh-web-app` + 本插件）在 `127.0.0.1:5227`，浏览器里发"谢谢" | `reasoningEffort: "off"` —— 尽管 settings 里默认是 `high`，且该 profile 装有 `installModelSelection` | `session-90909090` |
