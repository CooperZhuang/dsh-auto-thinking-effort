...
| **Real GUI turn** | temporary `web-verify` profile (`dsh-base` + `dsh-web-app` + this plugin) on `127.0.0.1:5227`, message sent from a browser | `reasoningEffort: "off"` — even though settings default to `high` and that profile installs `installModelSelection` | `session-90909090` |
