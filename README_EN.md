...
| **Real run: a card save reaches the host** | in that same process the card set `autoFloorLevel: low` and saved; a new session then sent `git status` | `settings.yaml` gained the value and that turn's `request/header` was `"low"` (the profile row alone gives `"off"` for that prompt) | `session-78787878` |
