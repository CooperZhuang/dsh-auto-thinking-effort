...
| **真机：卡片保存 → 运行中的 host 采纳（v0.6）** | 同一进程内用卡片把 `autoFloorLevel` 改成 `low` 保存，然后新会话发 `git status` | `settings.yaml` 写入成功（注释/其它段未动）；该轮 `request/header` = `"low"`（base 行单独跑同句是 `"off"`） | `session-78787878` |
