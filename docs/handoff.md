...
**settings 用户层示例**（示意值）：`~/.dsh/settings.yaml` 的 `agent-default-model` **没有** `reasoningEffort`（= 新会话默认 Auto，见 D12）。`web` profile 的 `cordis.patch.yml` 里有一条 `auto-thinking-effort` 覆盖行，把 `classifier` 设成 `model` + `deepseek-v4-flash`。
---
