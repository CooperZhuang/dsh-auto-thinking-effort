...
| 本机验证 profile | `C:\Users\<user>\.dsh\profiles\headless`（bundles: `dsh-base`, `dsh-headless`, **`dsh-auto-thinking-effort`**） |
| 用户设置 | `C:\Users\<user>\.dsh\settings.yaml`（`agent-default-model.reasoningEffort: high`） |
| 会话日志 | `C:\Users\<user>\.dsh\sessions\--C-...--\session-<id>\session.jsonl.zstd` |
...
| **对照组**（关插件） | 同句 + `--patch` 覆盖 `enabled: false` | `reasoningEffort: "high"` | `session-22222222` |
| 真机一轮（要求深想） | `dsh --profile headless "深入思考一下：…"` | `reasoningEffort: "max"` | `session-33333333` |
| 真机一轮（因果分析，未 pin） | `dsh --profile headless "Why does the upload helper return undefined after the migration?"` | `reasoningEffort: "high"`（why +5、migration +4 = 9） | `session-44444444` |
| **GUI 真机一轮** | 临时 `web-verify` profile（base+web-app+本插件）跑在 `127.0.0.1:5227`，浏览器发"谢谢" | `reasoningEffort: "off"`（settings 默认是 `high`，且该 profile 装 `installModelSelection`） | `session-90909090` |
