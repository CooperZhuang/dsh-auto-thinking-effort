...
**settings 用户层示例**（示意值）：`~/.dsh/settings.yaml` 的 `agent-default-model` 现在是 `deepseek-v4.1-flash-expires-on-0910` + **`reasoningEffort: high`**（用户在 GUI 里手动选了 High），所以**新会话现在走手动档**、插件不介入。想让新会话回到 Auto：在 GUI 里选一次 Auto（插件会把 effort 从落盘值里剥掉，见 D12），或删掉 settings.yaml 里那一行。
---
...
| 真机：下限生效（v0.3） | `dsh --profile headless "git status"` | `"low"`（v0.2 是 `off`） | `session-77777777` |
